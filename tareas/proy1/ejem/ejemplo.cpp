...
priority_queue< Node , vector<Node> , cmp > ColaPrioridad; //priority queue propia del c++, usamos el comparador definido para que el de menor valor este en el tope
...
void relajacion( int actual , int adyacente , int peso ) {
    ...
    if( distancia[ actual ] + peso < distancia[ adyacente ] ){
        ...
        ColaPrioridad.push( Node( adyacente , distancia[ adyacente ] ) ); //agregamos adyacente a la cola de prioridad
    }
}
...
void dijkstra( int inicial ) {
    ...
    ColaPrioridad.push( Node( inicial , 0 ) ); //Insertamos el vértice inicial en la Cola de Prioridad
...
int main(){
    ...
    while( E-- ){
        puts("Ingrese el origen | destino | peso");
