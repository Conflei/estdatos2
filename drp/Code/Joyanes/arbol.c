...
void dividirNodo(	Pagina* actual, tipoClave cl, Pagina* rd,
										 int k, tipoClave* mediana, Pagina** nuevo)
	{
int i, posMdna;
...
{
		/* Es desplazada la mitad derecha al nuevo nodo, la clave 
mediana se queda en el nodo origen */
...
if (encontrado)
{
	...
	if ((*raiz) -> cuenta == 0)
{
/* La raiz está vacía, se libera el nodo y se establece la 
 nueva raíz */
		Pagina* p = *raiz;
		*raiz = (*raiz) -> ramas[0];
		free(p);
